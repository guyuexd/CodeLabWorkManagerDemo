...
public class SaveImageToFileWorker extends Worker {
    ...
    private static final SimpleDateFormat DATA_FORMATTER = new SimpleDateFormat("yyyy.MM.dd 'at' HH:mm:ss z", Locale.getDefault());

    @NonNull
    @Override
    public Result doWork() {
        Context applicationContext = getApplicationContext();
        WorkerUtils.makeStatusNotificationi("Saving image...", applicationContext);
        WorkerUtils.sleep();
        ContentResolver resolver = applicationContext.getContentResolver();
        try {
            String resourceUri = getInputData().getString(Constants.KEY_IMAGE_URI);
            Bitmap bitmap = BitmapFactory.decodeStream(resolver.openInputStream(Uri.parse(resourceUri)));
            String imageUri = MediaStore.Images.Media.insertImage(resolver, bitmap, TITLE, DATA_FORMATTER.format(new Date()));
